...
deploy-app:
    ...
    {{python}} utils/smart_deploy.py --server {{server}} --remote-path {{remote_path}} --paths icon.svg icon.png favicon.ico requirements.txt bootstrap.sh "*.html" js css backend
...
deploy-app-including-unchanged:
    ...
    scp -r icon.svg icon.png favicon.ico requirements.txt bootstrap.sh *.html js css backend {{server}}:{{remote_path}}/
    ssh {{server}} "sudo systemctl restart {{service_name}}"
    @echo "App deployment complete."
